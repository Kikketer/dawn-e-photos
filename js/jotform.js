...
var JotForm = {
  ...
  encryptAll: function (e, callback) {
    ...
    $$('.form-textbox, .form-textarea, .form-radio, .form-checkbox, .form-dropdown, .form-number-input').each(function (
      field
    ) {
      ...
      if (
        (!isMultipleSelectionInput &&
          alreadyEncrypted.indexOf(encryptedFieldName) !== -1 &&
          !field.up().hasClassName('form-matrix-values') &&
          !field.up(1).hasClassName('form-matrix-values')) ||
  ...
  init: function (callback) {
      ...
      try {
        ...
        if (JotForm.showHIPAABadge) {
          ...
          if (!JotForm.enterprise) {
            hipaaWrapper.setAttribute(
              'href',
              'https://www.jotform.com/hipaa/?utm_source=formfooter&utm_medium=banner&utm_term=' +
                formID +
                '&utm_content=hipaa_compliant&utm_campaign=form_badges'
            )
    ...
    if (
      ...
      (this.jsForm && (document.readyState === undefined || document.readyState === 'interactive'))
    ) {
      ready()
    } else {
    ...
    p.$jot.getJSON(
      ...
      function (res) {
        ...
        if (ie !== undefined && ie < 9) {
          ...
          editorFrame.attr(
            'src',
            'https://screenshots.jotform.com/opt/templates/screen_editor.html?shot=' + url + '&uniq=' + $this.scuniq
          )
  ...
  initMultipleUploads: function () {
    ...
    if (JotForm.browserIs.mobile() && !isJotFormNext) {
      ...
      JotForm.loadScript(
        window.location.protocol + '//' + window.location.host + '/js/vendor/NoSleep.min.js?v_' + new Date().getTime(),
        function () {
      ...
      try {
        var uploader = JotForm.isFillingOffline()
          ...
          : new qq.FileUploader({
              ...
              sizeLimit:
                parseInt(file.readAttribute('data-file-maxsize') || file.readAttribute('file-maxsize'), 10) * 1024, // Set file size limit
              minSizeLimit:
                parseInt(file.readAttribute('data-file-minsize') || file.readAttribute('file-minsize'), 10) * 1024,
      ...
      if ($('lite_mode_' + id)) {
        $('lite_mode_' + id).dateChanged = function (e, calendar) {
          ...
          var inputLength = calendar.isNewTheme
            ? lite_mode.value.replace(new RegExp('[^\\d' + seperator + ']'), '').length
      ...
      if (other) {
        ...
        other.observe('click', function (event) {
          if (other.getAttribute('class').indexOf('[required]') > -1) {
            other_input.value =
              other_input.value === other_input.getAttribute('data-otherhint') ? '' : other_input.value
          } else {
            other_input.value =
    ...
    try {
      $$('[name$=[month]]').each(function (monthElement) {
        ...
        monthElement.dateTimeCheck = function (e) {
          ...
          var ignoreBirthdate =
            isBirthdate && (monthElement.value === '' || dayElement.value === '' || yearElement.value === '')
      ...
      if (names.length > 2) {
        if (middleName) {
          JotForm.fillFieldElementIfEmpty(
            middleName,
            user.name.substring(user.name.indexOf(' '), user.name.lastIndexOf(' '))
          )
        } else {
  ...
  prePopulations: function (fields) {
    ...
    $H(_data).each(function (pair) {
      ...
      if (!input && pair.key.indexOf('[') > 0) {
        ...
        if (name.length > 0 && $$('select[name*=' + name + '], input[name*=' + name + ']').length > 0) {
          ...
          if ($$('select[name*=' + name + '], input[name*=' + name + ']').length > index) {
            var type =
              $$('select[name*=' + name + ']').length > 0 ? 'select' : $$('input[name*=' + name + ']')[index].type
                  ...
                  if (
                    ...
                    !(
                      $$('input[name*=' + name + ']')
                        .first()
                        .up('.form-line')
                        .readAttribute('data-type') === 'control_matrix' && name.indexOf('[') < 0
                    )
                  ) {
      ...
      if (input && input.readAttribute('data-type') == 'input-grading') {
        ...
        input &&
        (input.hasClassName('form-checkbox-other-input') || input.hasClassName('form-radio-other-input'))
      ) {
        if (n.indexOf('[other]') > -1) {
          ...
          try {
            var valuesArray =
              input.up('.form-line').readAttribute('data-type') === 'control_checkbox'
                ? pair.value.split(',')
                : [pair.value]
        ...
        if (
          ...
          (formLine && dontStripPlusIn.indexOf(formLine.readAttribute('data-type')) > -1)
        ) {
          input.value = pair.value
        }
        JotForm.defaultValues[input.id] = input.value
      ...
      $$('.form-checkbox%s, .form-radio%s'.replace(/\%s/gim, n)).each(function (input) {
        ...
        if (
          value == input.value ||
          $A(value.split(',')).include(input.value) ||
          $A(value.split('<br>')).include(input.value)
        ) {
          if (!input.checked) {
            if (input.disabled) {
              // Ticket ID: 1479721
          ...
          if (action.fields) {
            action.fields.each(function (multiField) {
              if (
                multiField === field &&
                action.visibility &&
                action.visibility.toLowerCase().replace(/multiple/, '') === visibility &&
  ...
  showField: function (field, multiple) {
    ...
    if (
      !(element.hasClassName('form-section') || element.hasClassName('form-section-closed')) &&
  ...
  showWidget: function (id, postTranslate) {
    ...
    if (referrer) {
      var frame =
        navigator.userAgent.indexOf('Firefox') != -1 && typeof getIframeWindow !== 'undefined'
            ...
            if (el.readAttribute('data-type') === 'mixed-dropdown') {
              ...
              if (el.querySelector('input')) {
                el.querySelector('input').value =
                  dropdownID in JotForm.defaultValues ? JotForm.defaultValues[dropdownID] : ''
              }
            }
          })
      } else {
  ...
  getInputType: function (id) {
    ...
    if ($('id_' + id) && $('id_' + id).readAttribute('data-type') == 'control_text') {
      ...
      $('input_' + id + '_pick') ||
      ($('id_' + id) && $('id_' + id).readAttribute('data-type') == 'control_datetime')
    ) {
      type = 'datetime'
  ...
  checkCondition: function (condition) {
    ...
    $A(condition.terms).each(function (term) {
      ...
      try {
        ...
        switch (fieldType) {
          ...
          case 'address':
            if (['isEmpty', 'isFilled'].include(term.operator)) {
              filled = $$('#id_' + term.field + ' input:not(.jfDropdown-search)', '#id_' + term.field + ' select')
                .collect(function (e) {
                  return e.value
                })
                .any()
          ...
          case 'datetime':
            ...
            if (['isEmpty', 'isFilled'].include(term.operator)) {
              ...
              if (['equalDate', 'notEqualDate', 'after'].include(term.operator)) {
                ...
                if (
                  JotForm.checkValueByOperator(term.operator, JotForm.strToDate(termValue), JotForm.strToDate(value))
                ) {
                  any = true
                } else {
                  all = false
          ...
          case 'time':
            ...
            if (term.operator == 'before' && value.empty()) {
              ...
              if (JotForm.checkValueByOperator(term.operator, termValue, value)) any = true
              else all = false
            }
            break
          case 'checkbox':
          case 'radio':
            ...
            var otherValue = $('id_' + term.field).down('.form-' + fieldType + '-other-input')
              ? $('id_' + term.field)
            ...
            $$('#id_' + term.field + ' input').each(function (input) {
              ...
              if (checkResult) {
                ...
                if (term.operator == 'notEquals' && termValue == value) {
                  ...
                  (input.hasClassName('form-' + fieldType + '-other') && termValue == '-- ' + otherValue + ' --')
                ) {
                  all = false
                }
              }
          ...
          case 'widget':
            ...
            if (value.indexOf('widget_metadata') > -1) {
              ...
              value =
                (term.operator === 'greaterThan' || term.operator === 'lessThan') && typeof value === 'string'
                  ? value.replace(/,/g, '')
                  : value
    ...
    if (condition.link.toLowerCase() == 'any' && any) {
      ...
      if (
        ...
        anyCalculationForMultiInput.notEqualsCount + anyCalculationForMultiInput.equalsCount
      ) {
    ...
    if (condition.type == 'field') {
      ...
      var isConditionValid =
        (condition.link.toLowerCase() == 'any' && any) || (condition.link.toLowerCase() == 'all' && all)
          ...
          function (calc) {
            ...
            for (var i = 0; i < calcs.length; i++) {
              if (
                ...
                !(
                  JotForm.getInputType(condition.action[0].resultField) === 'html' &&
              ...
              try {
                ...
                if (!dontTrigger) {
                  dontTrigger =
                    ...
                    condition.action[0].operands.split(',').include(condition.action[0].resultField)
                }
        ...
        if (currentQuestionIndex < nextQuestionIndex) {
          for (var i = currentQuestionIndex + 1; i < nextQuestionIndex; i++) {
            ...
            if (
              ...
              card.getAttribute('data-skipped-by') !== allQuestions[currentQuestionIndex].id
            ) {
  ...
  checkCalculation: function (calc) {
    ...
    var useCommasForDecimals =
      calc.useCommasForDecimals && calc.useCommasForDecimals != '0' ? calc.useCommasForDecimals : false
    ...
    var getValue = function (data, numeric) {
      ...
      switch (type) {
        ...
        case 'time':
          ...
          var valArr = []
          combinedObject = {}
          var timeElements = JotForm.newDefaultTheme
            ? '#id_' + data + ' select,' + '#id_' + data + ' input,' + ', #id_' + data + ' input:not(.form-textbox)'
            : '#id_' + data + ' select'
        ...
        case 'address':
          ...
          var dropdownSelector =
            '#id_' + data + (window.FORM_MODE == 'cardform' ? ' .jfField:not(.isHidden)' : '') + ' select'
    ...
    if (
      ...
      output =
        $('input_' + result).readAttribute('defaultValue') || $('input_' + result).readAttribute('data-defaultvalue')
    }
        ...
        if (result.indexOf('|') > -1) {
          ...
          Object.keys(combinedObject).forEach(function (id) {
            var selector = isNewIDType
              ...
              : '#id_' + qid + ' input[id*=' + id + '][id$=-' + fieldId + ']'
            ...
            if (resultFieldType === 'inline') {
              ...
              if (calc.isLabel) {
                contents = contents.replace(
                  re,
                  '<span class="replaceTag ' + className + '" default="' + def + '">' + output + '</span>'
                )
              } else {
        ...
        if ((combinedObject && 'year' in combinedObject) || resultFieldType === 'time') {
              ...
              for (var inputId in combinedObject) {
                if (JotForm.isTimeObject(inputId)) {
                  var ndtTimeQuery = $('id_' + result).select(
                    'input[id*=input_' + result + '_' + inputId + '], select[id*=input_' + result + '_' + inputId + ']'
                  )
                  ...
                  targetTimeObject =
                    !JotForm.newDefaultTheme && ldtTimeQuery.length > 0 ? ldtTimeQuery.first() : targetTimeObject

                  var cardFormTimeQuery = $('id_' + result).select(
                    'input[id*=' + inputId + '_' + result + '], select[id*=' + inputId + '_' + result + ']'
                  )
                  targetTimeObject =
                    window.FORM_MODE == 'cardform' && cardFormTimeQuery.length > 0
          ...
          try {
            if (
              ...
              (typeof output == 'string' && output.replace(/\s/g, '').length > 0 && output !== '0')
            ) {
              if (output.length === 13) {
                output = parseFloat(output, 10)
    ...
    if (!mixedResult && $('id_' + result).hasClassName('form-line-error')) {
      $('id_' + result)
        .select("select[class*='required'], textarea[class*='required'], input[class*='required']")
        .each(function (el) {
          if (el.validateInput) {
            el.validateInput()
          }
  ...
  setFieldsToPreserve: function (preset) {
    ...
    var getPaymentFields =
      ...
      gateways.indexOf($$('input[name="simple_fpc"]')[0].getAttribute('data-payment_type')) > -1
  ...
  handleBraintree: function () {
    ...
    if (
      window.location.pathname.match(/^\/edit/) ||
      (['edit', 'inlineEdit', 'submissionToPDF'].indexOf(document.get.mode) > -1 && document.get.sid)
    ) {
      return
    }
  ...
  handleSquare: function () {
    // skip on edit mode
    if (
      /*JotForm.paidSubmission && */ (window.location.href.match(/mode=inlineEdit/) ||
        window.location.pathname.match(/^\/\/edit/) ||
        window.location.pathname.match(/^\/edit/) ||
        window.location.href.match(/mode=submissionToPDF/)) &&
      document.get.sid
  ...
  handlePaymentSubProducts: function () {
    ...
    $$('.form-product-has-subproducts').each(function (sp) {
      var wasHidden =
        sp.up('.form-line') && sp.up('.form-line').hasClassName('form-field-hidden')
    ...
    function showSubProducts(el) {
      ...
      if (!el.checked) {
        ...
        if (
          ...
          productSpan.querySelector('.form-product-description').childNodes[0] &&
          productSpan.querySelector('.form-product-description').childNodes[0].nodeValue.trim() !== ''
        ) {
          productSpan.shift({
  ...
  clearProductValues: function (el, optionValues) {
    $$('#' + el.id + '_subproducts select,' + '#' + el.id + '_subproducts input[type="text"]').each(function (
      field,
      i
    ) {
      // capture the values
    ...
    $H(prices).each(function (pair) {
      ...
      if (!itemName) {
        itemName =
          ...
          $$('#' + label + '+ .product__header .product__title')[0].textContent.trim()
      }
      ...
      if (!pair.value.recurring) {
        var priceText = $(pair.key + '_price')
          ? $(pair.key + '_price')
          : $(pair.key.replace(pair.key.substring(pair.key.lastIndexOf('_')), '') + '_price') || null
        if (priceText) {
      ...
      if (pair.value.tax) {
        ...
        var locationField =
          $$('select[id*="input_' + tax.surcharge.field + '"], input#input_' + tax.surcharge.field)[0] || false
        ...
        if ($(pair.value.quantityField) || $(pair.value.specialPriceField)) {
          ...
          if ($(pair.value.quantityField) && pair.value.specialPriceField !== pair.value.quantityField) {
            ...
            if ($(pair.value.quantityField).readAttribute('type') == 'text') {
              price = $(pair.value.quantityField).value
                ? price * Math.abs(parseInt($(pair.value.quantityField).value, 10))
                : 0
              priceWithoutDiscount = $(pair.value.quantityField).value
                ? priceWithoutDiscount * Math.abs(parseInt($(pair.value.quantityField).value, 10))
                : 0
              ...
              priceWithoutDiscount =
                priceWithoutDiscount * parseInt($(pair.value.quantityField).getSelected().text || 0, 10)
          ...
          if ($(parentProductKey + '_item_subtotal') && !isNaN(price)) {
            if (!subproduct) {
              ...
              $(parentProductKey + '_item_subtotal').update(
                parseFloat(itemSubTotal[parentProductKey]).formatMoney(decimal, dSeparator, tSeparator)
              )
            }
          }
          ...
          if (shipping.firstItem) {
            ...
            if (qty > 1) {
              productShipping = !parseFloat(shipping.addItem)
                ...
                : parseFloat(shipping.firstItem) + parseFloat(shipping.addItem) * (qty - 1)
            }
      ...
      if ($(pair.key) || JotForm.couponApplied) {
        if (
          ...
          window.paymentType === 'subscription' &&
          Array.from(document.querySelectorAll('.jfCard')).filter(function (el) {
          ...
          if ($(input).id.indexOf('_quantity_') > -1 && (JotForm.newDefaultTheme || JotForm.newPaymentUI)) {
            ...
            setTimeout(function () {
              var connected_product_item_input = $$(
                '.form-product-item[pid="' + connectedProductPid + '"] .form-product-input'
              )[0]
      ...
      cb.observe(
        ...
        function () {
          if (ci.value) {
            ...
            var isStripe =
              (ci.hasAttribute('stripe') || ci.hasAttribute('data-stripe')) && window.paymentType === 'subscription'
    ...
    if (window.discounted_products) {
      ...
      discounted_products.forEach(function (element, index) {
        if (typeof element === 'string' || typeof element === 'number') {
          ...
          if (
            cleared_discounted_products.includes(selected_product_id) === true ||
  ...
  applyCoupon: function (discount) {
    ...
    var displayOldPrice = function (container, id) {
      ...
      var span = new Element('span', { style: 'text-decoration:line-through; display:inline-block;' }).addClassName(
        'old_price'
      )
      ...
      if (discount.apply === 'product') {
        $A(discount.products).each(function (pid) {
          ...
          if (
            ...
            $$('[id*=' + pid + '_subproducts]')[0].down('.form-product-child-price')
          ) {
            $$('#' + $$('[id*=' + pid + '_subproducts]')[0].id + ' .form-product-child-price').each(function (
              field,
              id
            ) {
  ...
  highLightLines: function () {
    ...
    $$('.form-line').each(function (l, i) {
      l.select('input, select, textarea, div, table div, button, div div span[tabIndex]:not([tabIndex="-1"])').each(
        function (i) {
          i.observe('focus', function () {
            if (
              l.parentElement &&
              (l.parentElement.hasClassName('form-section') || l.parentElement.hasClassName('form-section-closed'))
            ) {
  ...
  getSection: function (element) {
    ...
    if (
      (element.hasClassName('form-section-closed') || element.hasClassName('form-section')) &&
      ...
      b.onclick = function () {
        ...
        setTimeout(function () {
          $$('.custom-hint-group').each(function (element) {
            ...
            element.hasContent =
              element.value && element.value.replace(/\n/gim, '<br>') != element.readAttribute('data-customhint')
                ? true
                : false
      ...
      print_button.observe('click', function () {
        ...
        $$('.form-textarea, .form-textbox').each(function (el) {
          ...
          if (
            ...
            (el.id.indexOf('_country') != -1 && el.readAttribute('type') == 'tel')
          ) {
  ...
  hasMinTotalOrderAmount: function () {
    ...
    var hasMinTotalOrderAmount =
      typeof minTotalOrderAmountHiddenField !== 'undefined' && minTotalOrderAmountHiddenField.length > 0
  ...
  handleMinTotalOrderAmount: function () {
    if (
      ...
      JotForm.paymentTotal < JotForm.minTotalOrderAmount
    ) {
      var errorTxt = JotForm.texts.ccDonationMinLimitError
  ...
  hasHiddenValidationConflicts: function (input) {
    ...
    return (
      hiddenOBJ &&
      (hiddenOBJ.hasClassName('form-field-hidden') || hiddenOBJ.up('ul.form-section').hasClassName('form-field-hidden'))
    )
  },
  ...
  renderTermsAndConditions: function (settings) {
    ...
    function sendData() {
      ...
      widgetIframe.postMessage(
        { data: 'termsAndConditions', settings: { qid: settings.data.settings.qid, status: 'accepted' } },
        '*'
      )
      closeModel()
    }
  ...
  handleAuthNet: function () {
    ...
    Event.observe(thisForm, 'submit', function (event) {
      ...
      if (JotForm.isPaymentSelected() && JotForm.paymentTotal > 0) {
        var errors
        $$('#id_' + paymentFieldId + ' input[class*="cc"]', '#id_' + paymentFieldId + ' select[class*="cc"]').each(
          function (cc) {
            if (!cc.getValue()) {
  ...
  handleFITBInputs: function () {
    function getInputWidth(fitbInput) {
      ...
      textElement.style.cssText =
        'position: absolute; font-size:' + fitbFontSize + '; font-family:' + fitbFontFamily + '; display: inline-block;'
    ...
    var visibleFitbInputs = Array.from(
      document.querySelectorAll('.FITB input:not([type="checkbox"]):not([type="radio"])')
    ...
    if ($('creditCardTable')) {
      ...
      Event.observe(thisForm, 'submit', function (event) {
        ...
        if (JotForm.isPaymentSelected() && JotForm.paymentTotal > 0) {
          ...
          var isSubscription =
            document.getElementById('payment-wrapper-songbird').getAttribute('data-paymenttype') === 'subscription'
          var is3DSecurityEnabled =
            document.getElementById('payment-wrapper-songbird').getAttribute('data-sca') === 'Yes'
              ...
              Cardinal.on('payments.validated', function (data, jwt) {
                ...
                switch (data.ActionCode) {
                  ...
                  case 'NOACTION':
                    ...
                    var ECIFlag =
                      data.Payment.ExtendedData.ECIFlag !== undefined ? data.Payment.ExtendedData.ECIFlag : ''
      ...
      $$('.paymentTypeRadios').each(function (radio) {
        radio.observe('click', function () {
          ...
          if (
            radio.checked &&
            radio.value === 'credit' &&
            (JotForm.paymentTotal > 0 || Object.keys(JotForm.discounts).length === 0)
          ) {
            JotForm.setCreditCardVisibility(true)
          }
          JotForm.corrected($$('.paymentTypeRadios')[0])
    ...
    if (window.signatureForm) {
      ...
      for (var i = 0; i < pads.length; i++) {
        ...
        if (jQuery(pad).attr('data-required') === 'true') {
          ...
          if (
            formLine.is(':visible') &&
            JotForm.getSection(pad).hasClassName('js-non-displayed-page') === false &&
  ...
  errored: function (input, message) {
    ...
    insertEl.insert(
      new Element('div', {
        ...
        .insert('<img src="' + preLink + 'images/exclamation-octagon.png"> ' + message)
        .insert(
  ...
  corrected: function (input) {
    ...
    if (JotForm.isCollapsed(input)) {
      ...
      if (
        ...
        collapse.up('.form-section-closed').select('.form-validation-error').length == 0
      ) {
        collapse
          .select('.form-collapse-mid')[0]
          .setStyle({
    ...
    $A(JotForm.forms).each(function (form) {
      ...
      form.observe('submit', function (e) {
        ...
        if (JotForm.clearFieldOnHide !== 'dontClear') {
          $$('.form-field-hidden input', '.form-field-hidden select', '.form-field-hidden textarea').each(function (
            input
          ) {
          ...
          JotForm.encryptAll(e, function (submitForm) {
            if (submitForm) {
              ...
              if (Object.keys(redirectConditions).length > 0) {
                form.insert(
                  new Element('input', { type: 'hidden', name: 'redirectConditions' }).putValue(
  ...
  setFieldValidation: function (input) {
    ...
    input.validateInput = function (deep, dontShowMessage) {
      ...
      if (
        JotForm.getContainer(input).getAttribute('data-type') === 'control_datetime' &&
      ...
      if (
        JotForm.getContainer(input).getAttribute('data-type') === 'control_time' &&
      ...
      if (vals.include('minSelection') || vals.include('minselection')) {
        ...
        if (numberChecked > 0 && numberChecked < minSelection) {
          return JotForm.errored(
            input,
            JotForm.texts.minSelectionsError.replace(/\s*$/, '') + ' ' + minSelection + '.',
            dontShowMessage
          )
        }
      }
      ...
      if (vals.include('maxselection')) {
        ...
        if (numberChecked > maxSelection) {
          return JotForm.errored(
            input,
            JotForm.texts.maxSelectionsError.replace(/\s*$/, '') + ' ' + maxSelection + '.',
            dontShowMessage
          )
        }
      }
      ...
      if (vals.include('limitDate')) {
        try {
          ...
          if (
            lim !== false &&
            !($('year_' + id).value == '' || $('month_' + id).value == '' || $('day_' + id).value == '')
          ) {
            //custom
            if ('custom' in lim && lim.custom !== false && Array.isArray(lim.custom)) {
              for (var j = 0; j < lim.custom.length; j++) {
                ...
                if (
                  ...
                  (typeof lim.custom[j] == 'string' && lim.custom[j].length === 2 && lim.custom[j] == day)
                ) {
                  //day
                  return JotForm.errored(input, JotForm.texts.dateLimited, dontShowMessage)
                }
      ...
      if (vals.include('required')) {
        ...
        if (input.tagName == 'INPUT' && input.readAttribute('type') == 'file') {
          ...
          var otherInput = input.up('.form-' + input.type + '-item')
            ? input.up('.form-' + input.type + '-item').down('.form-' + input.type + '-other-input')
            : null
          if (otherInput) {
                ...
                !$A(document.getElementsByName(input.name))
                  .map(function (e) {
                    if (JotForm.isVisible(e)) {
                      ...
                      if (
                        (e.readAttribute('type') === 'checkbox' || e.readAttribute('type') === 'radio') &&
                        ...
                        if (!e.checked) {
                          ...
                          return $A($$('#' + e.id + '_subproducts .form-subproduct-quantity'))
                            .map(function (cb) {
                        ...
                        var qty = e.up('.form-product-item')
                          ? e.up('.form-product-item').down('select[id*="quantity"], input[id*="quantity"]')
                          : false
                        if (qty) {
                  ...
                  .any() ||
                (window.FORM_MODE === 'cardform' && cont.select('.product--selected').length === 0)
              ) {
          ...
          ($(input).up().hasClassName('form-matrix-values') || $(input).up(1).hasClassName('form-matrix-values'))
        ) {
          ...
          if (vals.include('requireEveryRow') && !hasOneAnswerEveryRows) {
            ...
            (allCells.map(isInputTypeRadioOrCheckbox).every()
              ? !hasOneAnswerEveryRows
              : !allCells.map(isFilled).every())
          ) {
            if (JotForm.newDefaultTheme) {
              ...
              for (var i = 0; i < allChildren.length; i++) {
                if (
                  (allChildren[i].getAttribute('type') === 'radio' ||
          ...
          $this.getContainer(input).getAttribute('data-type') === 'control_inline'
        ) {
          try {
            ...
            var checkValues = inputs.map(function (e) {
              ...
              if (e.id.match(/input_[0-9]+_quantity_[0-9]+_[0-9]+/)) {
                ...
                var allProducts = $$(
                  '[id*="' + e.id.match(/input_[0-9]*/)[0] + '"][type="' + cb.getAttribute('type') + '"]'
                )
                ...
                if (
                  (cb.checked && !subProductQty && (isNaN(e.value) || e.value == 0 || e.value.empty())) || // if a product is selected and qty is not valid
                  !allProducts
                    .map(function (c) {
                      return c.checked
                    })
              ...
              if (innerVals.include('required') && JotForm.isVisible(e)) {
                if (
                  JotForm.getContainer(e).getAttribute('data-type') === 'control_inline' &&
        ...
        if (input.hasClassName('form-textarea') && input.up('div').down('.nicEdit-main')) {
          ...
          if (
            ...
            (input.readAttribute('data-customhint') &&
              input.readAttribute('data-customhint') == input.up('div').down('.nicEdit-main').innerHTML)
          ) {
            return JotForm.errored(input, JotForm.texts.required, dontShowMessage)
          }
      ...
      setTimeout(function () {
        ...
        try {
          ...
          var isLastRequiredField = $this.lastFocus
            ? $this.getContainer($this.lastFocus) == $this.getContainer(input) && lastRequiredField == input
            : false
          ...
          if (
            ...
            ($this.lastFocus == input ||
              $this.getContainer($this.lastFocus) != $this.getContainer(input) ||
              isLastRequiredField) &&
            !isPrefix &&
            !(window.FORM_MODE === 'cardform' && inputContainer.dataset.type === 'control_fullname') &&
            !(
              window.FORM_MODE === 'cardform' &&
              (inputContainer.dataset.type === 'control_checkbox' || inputContainer.dataset.type === 'control_radio')
            )
          ) {
            input.validateInput()
  ...
  fieldHasContent: function (id) {
    ...
    switch (type) {
      ...
      case 'file':
        if (
          $$('#id_' + id + ' input')[0].readAttribute('multiple') === 'multiple' ||
  ...
  setupRichArea: function (qid) {
    if (!(!Prototype.Browser.IE9 && !Prototype.Browser.IE10 && Prototype.Browser.IE)) {
      ...
      new nicEditor({ iconsPath: location.protocol + '//www.jotform.com/images/nicEditorIcons.gif?v2' }).panelInstance(
        'input_' + qid
      )
  ...
  autoFillInitialize: function (params) {
    ...
    if (JotForm.payment && $$('.form-product-item > input.form-product-has-subproducts').length > 0) {
      $$('.form-line[data-type="control_authnet"] select, .form-line[data-type="control_authnet"] input').each(
        function (input) {
          if (input.id) {
      ...
      if ($(matrix).select('.forDesktop').length > 0) {
        ...
        if (headerItems && headerItems.length) {
          headerItems[0].getElementsByTagName('div')[0].style.width =
            tableCells[0].getElementsByTagName('div')[0].getLayout().get('padding-box-width') + 'px'
      ...
      if (hasMobile) {
        ...
        var handleNextButtonClick = function () {
          ...
          var activeQuestionOrder = parseInt(
            $(matrix).select('.jfMatrix-question.isActive')[0].readAttribute('data-order')
          )
        ...
        $(matrix).select('.forMatrixNext').length > 0 &&
          $(matrix).select('.forMatrixNext')[0].observe('click', handleNextButtonClick)
        $(matrix).select('.forMatrixPrev').length > 0 &&
          $(matrix).select('.forMatrixPrev')[0].observe('click', handlePrevButtonClick)
              ...
              if (input.type == 'radio' && dataType !== 'Yes No') {
                var showNextQuestion = function () {
                  ...
                  if (
                    ...
                    $(activeTable).select('.jfMatrixChoice-row.isSelected').length > 0
                  ) {
  ...
  serialize: function (data) {
    var str = []
    for (var p in data) str.push(encodeURIComponent(p) + '=' + encodeURIComponent(data[p]))
    return str.join('&')
  },
  ...
  initPrefills: function () {
    ...
    var url =
      ...
      '//' +
      window.location.hostname +
      '/server.php?action=getPrefillData&formID=' +
      _form.id +
      '&key=' +
      document.get.prefillToken
    var data = {}
    JotForm.createXHRRequest(
      url,
      'get',
